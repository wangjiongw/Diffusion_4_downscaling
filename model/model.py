...
class DDPM(BaseModel):
    ...
    def __init__(
        ...
        noise_predictor = define_network(
            in_channel,
            out_channel,
            norm_groups,
            inner_channel,
        ...
        if self.phase == "train":
            self.register_schedule(
                beta_schedule=train_schedule,
                timesteps=train_n_timestep,
                linear_start=train_linear_start,
                linear_end=train_linear_end,
            )
        else:
            self.register_schedule(
                beta_schedule=val_schedule,
                timesteps=val_n_timestep,
                linear_start=val_linear_start,
                linear_end=val_linear_end,
            )
            ...
            if self.finetune_norm:
                ...
                for k, v in self.SR_net.named_parameters():
                    ...
                    if k.find("norm") >= 0:
                        ...
                        logger.info(
                            f"Params [{k:s}] initialized to 0 and will be fine-tuned."
                        )
            else:
                optim_params = list(self.SR_net.parameters())

            self.optimizer = optimizer(
                optim_params, lr=learning_rate, amsgrad=amsgrad, weight_decay=0.0
            )

            # Learning rate schedulers.
            self.scheduler = CosineAnnealingWarmRestarts(
                self.optimizer, T_0=100000, eta_min=1e-6
            )
    ...
    def batch2patch(self, data, batch):
        ...
        for i in range(batch):
            ...
            temp_data = data[
                i * len(self.index_list) : (i + 1) * len(self.index_list), :, :, :
            ]
    ...
    def infer_patch_v2(
        ...
        self.SR_net.eval()
        with torch.no_grad():
            if isinstance(self.SR_net, nn.parallel.DistributedDataParallel):
                SR_temp = self.SR_net.module.sample(
                    input_data,
                    return_intermediates=continuous,
                    ddim=use_ddim,
    ...
    def infer_generate_multiple_candidates_v2(
        self, n: int = 10, use_dpm_solver=False, ddim_steps=200
    ) -> torch.tensor:
    ...
    def infer_generate_multiple_candidates(
        self, n: int = 10, use_ddim=True, use_dpm_solver=False, ddim_steps=200
    ) -> torch.tensor:
        """Generates n super-resolution tesnors.
        Args:
            n: The number of candidates.
    ...
    def test(
        ...
        with torch.no_grad():
            if isinstance(self.SR_net, nn.parallel.DistributedDataParallel):
                self.SR = self.SR_net.module.sample(
    ...
    def generate_multiple_candidates(
        self, n: int = 10, ddim_steps=200, use_dpm_solver=False
    ) -> torch.tensor:
        """Generates n super-resolution tensors.
        Args:
            n: The number of candidates.
        Returns:
            n super-resolution tensors of shape [n, B, C, H, W] corresponding
    ...
    def register_schedule(
        self, beta_schedule, timesteps, linear_start, linear_end
    ) -> None:
        ...
        """
        if isinstance(self.SR_net, nn.parallel.DistributedDataParallel):
            self.SR_net.module.register_schedule(
                None,
                beta_schedule,
                timesteps,
                linear_start,
    ...
    def get_current_visuals(
        self, need_LR: bool = True, only_rec: bool = False
    ) -> typing.OrderedDict:
        ...
        if only_rec:
            ...
            out_dict["INTERPOLATED"] = (
                self.data["INTERPOLATED"][:, 0:1].detach().float().cpu()
            )
    ...
    def print_network(self) -> None:
        ...
        if isinstance(self.SR_net, nn.parallel.DistributedDataParallel):
            net_struc_str = "{} - {}".format(
                self.SR_net.__class__.__name__, self.SR_net.module.__class__.__name__
            )
        else:
    ...
    def load_network(self) -> None:
        ...
        if self.resume_state is not None and len(self.resume_state) > 0:
            logger.info(f"Loading pretrained model for G [{self.resume_state:s}] ...")
            gen_path, opt_path = (
                f"{self.resume_state}_gen.pth",
                f"{self.resume_state}_opt.pth",
            )
            ...
            network.load_state_dict(
                torch.load(gen_path), strict=(not self.finetune_norm)
            )
