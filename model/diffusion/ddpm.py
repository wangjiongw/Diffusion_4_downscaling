...
class DDPM(nn.Module):
    ...
    def __init__(
        ...
        self.device = torch.device(
            "cuda" if torch.cuda.is_available() and self.gpu_ids else "cpu"
        )
    ...
    def set_loss(self, device: torch.device):
        ...
        if self.loss_type == "l1":
            ...
            raise NotImplementedError(
                "Specify loss_type attribute to be either 'l1' or 'l2'."
            )
    ...
    def p_mean_variance(
        self, x, t, clip_denoised: bool, condition_x: torch.Tensor = None
    ):
    ...
    def sample(
        ...
        if ddim:
            ...
            return self.p_sample_loop(x_in, return_intermediates=return_intermediates)
    ...
    def forward(self, x: dict, *args, **kwargs):
        ...
        t = torch.randint(
            0, self.num_timesteps, (x["INTERPOLATED"].shape[0],), device=self.device
        ).long()
        # print("t:",t.shape)
