...
from data.mydataset_patch import SR3_Dataset_finetune_patch, SR3_Dataset_patch
...
if __name__ == "__main__":
    ...
    train_data = SR3_Dataset_finetune_patch(
    ...
    val_data = SR3_Dataset_finetune_patch(
        np.array(target_paths)[random_dataset_index],
        land_01_path,
        mask_path,
        lr_paths=np.array(lr_paths)[random_dataset_index],
        var=variable_name,
        patch_size=configs.height,
    )
    ...
    val_loader = DataLoader(
        val_data,
        batch_size=np.int(configs.batch_size / 12),
        shuffle=False,
        num_workers=configs.num_workers,
        drop_last=True,
    )
                ...
                for val_data in val_loader:
                    ...
                    if idx % configs.val_vis_freq == 0:
                        ...
                        sr_candidates = diffusion.generate_multiple_candidates(
                            n=configs.sample_size, ddim_steps=100, use_dpm_solver=False
                        )
                        ...
                        random_idx = np.random.randint(
                            0, np.int(configs.batch_size / 12), 5
                        )
                        ...
                        for idx_i, num in enumerate(random_idx):
                            ...
                            axs[idx_i, 8].imshow(
                                val_data["mask"][num, 0], vmin=0, vmax=2, cmap="RdBu_r"
                            )
