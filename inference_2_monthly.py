...
def loop_prediction(start_year, end_year, ddim_steps):
    ...
    for year in range(start_year, end_year):
        ...
        data_paths = sorted(
            glob.glob(
                "/home/data/downscaling/downscaling_1023/data/test_dataset/sl/*{0}_monthly*npy".format(
                    year
                )
            )
        )
        information = xr.open_dataset(
            "/home/data/downscaling/downscaling_1023/data/ERA_deal/different_grid/10km/ERA5_land_10km_East_china_{0}_monthly.nc".format(
                year
            )
        )
        val_logger.info(f"Dataset- Testing] is created=========year: " + str(year))
        val_dataset = SR3_Dataset_all(
            land_01_path, mask_path, lr_paths=data_paths, var=variable_name
        )
        val_loader = DataLoader(
            val_dataset, batch_size=batch, shuffle=False, num_workers=3
        )
        idx = 0
        with torch.no_grad():
            ...
            if need_member:
                if variable_name == "tp":
                    new_member_data = torch.clamp(
                        torch.cat(member_data, dim=1), 0, 5
                    ).numpy()
                    new_member_data = np.exp(new_member_data[:, :, 0, :, :]) - 1
                    new_member_data[new_member_data < 0] = 0
                else:
                    new_member_data = torch.clamp(
                        torch.cat(member_data, dim=1), 0, 1
                    ).numpy()
                    new_member_data = (
                        new_member_data[:, :, 0, :, :] * (max_normal - min_normal)
                        + min_normal
                    )
                    new_member_data = (
                        new_member_data * std_hr[list_idx] + mean_hr[list_idx]
                    )
        ...
        dataset_new.to_netcdf(
            result_path + "/" + "single_results/predict_{0}_.nc".format(year)
        )
        ...
        if need_member:
            ...
            dataset_new.to_netcdf(
                result_path + "/" + "multi_member/predict_{0}_.nc".format(year)
            )
...
if __name__ == "__main__":
    ...
    if variable_name == "tp":
        ...
        mean_hr = np.load(
            "/home/data/downscaling/downscaling_1023/data/train_dataset/mean&std/hr_mean.npy"
        ).transpose(2, 0, 1)

        std_hr = np.load(
            "/home/data/downscaling/downscaling_1023/data/train_dataset/mean&std/hr_std.npy"
        ).transpose(2, 0, 1)
    ...
    val_logger.info(
        f"Testing the model at epoch: {current_epoch}, iter: {current_step}."
    )

    diffusion.register_schedule(
        beta_schedule=configs.test_schedule,
