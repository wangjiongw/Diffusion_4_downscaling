...
class SR3_Dataset_patch(torch.utils.data.Dataset):
    def __init__(self, hr_paths, land_paths, mask_paths, lr_paths, var, patch_size):
        ...
        for index, memmap in enumerate(self.target_hr):
            ...
            self.data_count += memmap.shape[0]  # N
    ...
    def get_patch(self, hr, mask, hr_land, lr_inter):
        ...
        ip = self.patch_size  # HR patch size
        ...
        if self.variable_name in ["u", "v", "t2m", "sp"]:
            lr_data = self.normal_max_min(
                lr_inter[:, iy : iy + ip, ix : ix + ip].float(), iy, ix, ip
            )
            ret = {
                "HR": self.normal_max_min(
    ...
    def __getitem__(self, index):
        memmap_index = bisect(self.start_indices, index) - 1  # group ID
        index_in_memmap = index - self.start_indices[memmap_index]  # sample ID in group

        land_01_data = self.land_01
        mask_data = self.mask_data
        hr_target = self.target_hr[memmap_index][index_in_memmap] * mask_data  # C, H, W
...
if __name__ == "__main__":
    data_paths = sorted(
        glob.glob("/home/data/downscaling/downscaling_1023/data/train_dataset/sl/*npy")
    )
    target_paths = sorted(
        glob.glob("/home/data/downscaling/downscaling_1023/data/train_dataset/hr/*npy")
    )
    physical_paths = sorted(
        glob.glob("/home/data/downscaling/downscaling_1023/data/train_dataset/pl/*npy")
    )
